...
namespace ilrd
{
    ...
    {
        if (m_creation_funcs.end() != m_creation_funcs.find(key))
        {
    ...
    template <class Base, class Key, class Args>
    const char *Factory<Base, Key, Args>::KeyDuplicateException::what()
        const noexcept
    {
    ...
    template <class Base, class Key, class Args>
    const char *Factory<Base, Key, Args>::KeyDoesntExistException::what()
        const noexcept
    {

    ...
    if (m_thread_queue.size() > n_threads)
    {
             ...
             thread_list_runner = (thread_list_runner != m_thread_queue.end()) ? thread_list_runner : m_thread_queue.begin())
        {
            if (PAUSE == thread_list_runner->get()->GetStatus())
            {
                ...
                if (KILL != thread_list_runner->get()->GetStatus())
                {
                    ++thread_list_runner;
                    continue;
                }
...
void ThreadPool::ThreadUnit::ThreadFunc()
{
    ...
    do
    {
        ...
        while ((false != g_is_threads_running) && (KILL != GetStatus()))
        {
            SetStatus(RUN);
            if ((RUN != GetStatus()))
            {
...
{
    std::unique_lock<std::mutex> ul(m_status_lock);
...
void ThreadPool::ThreadUnit::SetStatus(THREAD_STATUS status)
{
    ...
    if ((status == KILL) && (m_thred_status == RUN))
    {
        return;
    }

    if ((status == RUN) && (m_thred_status == KILL))
    {
        return;
    }

    if ((status == PAUSE) && (m_thred_status == KILL))
    {
        return;
    }

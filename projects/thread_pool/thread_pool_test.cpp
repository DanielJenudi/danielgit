...
try
{
    PRINT << "PauseLocked" << END;
    PauseLocked();
    PRINT << "PopLocked" << END;
    PopLocked();
    PRINT << "Intrude" << END;
    Intrude();
    PRINT << "SetThreadsRemoveWhilePause" << END;
    SetThreadsRemoveWhilePause();
    PRINT << "SetThreadsRemoveWhileRun" << END;
    SetThreadsRemoveWhileRun();
    PRINT << "Stop" << END;
    Stop();
    PRINT << "SetNum" << END;
    SetNum();
    PRINT << "SimulateLogger" << END;
    SimulateLogger();
...
{
    ThreadPool A(1);
    A.Add(std::shared_ptr<Task>(new Default_Task(foo4)), LOW);
    A.SetNumberOfThreads(0);
    A.Run();
}
